...
region=$(get_region)
echo aws s3api create-bucket $(get_region_opt) --bucket $bucket --create-bucket-configuration LocationConstraint=$region
aws s3api create-bucket $(get_region_opt) --bucket $bucket --create-bucket-configuration LocationConstraint=$region
on_error Cannot create bucket

aws s3 website $(get_region_opt) s3://$bucket --index-document index.html --error-document error.html
on_error Cannot configure bucket for static website hosting

aws s3api delete-public-access-block $(get_region_opt) --bucket $bucket
...
aws s3api put-bucket-policy $(get_region_opt) --bucket $bucket --policy file://$home/policy.json
on_error Cannot apply policy

aws s3api put-bucket-cors $(get_region_opt) --bucket $bucket --cors-configuration '{
    "CORSRules": [
        {
            "AllowedOrigins": ["*"],
